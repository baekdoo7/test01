...
$sql = "  select search_date
           ...
           where search_date  >= '".$now."'
             and search_date < '".$tmr."'
           group by search_date, search_engine, site_idx
...
//    $vals_tmp[] = sprintf("('%s','%s','%s','%s','%s') ",$rows['search_date'],$rows['cnt'],$rows['pc_cnt'],$rows['m_cnt'],$rows['site_idx']);
    $vals_tmp[] = sprintf("('%s','%s','%s','%s','%s','%s') ",$rows['search_date'],$rows['search_engine'],$rows['cnt'],$rows['pc_cnt'],$rows['m_cnt'],$rows['site_idx']);
...
//    $strSQL = " INSERT INTO basic_refer_keyword_count (search_date, cnt, pc_cnt, m_cnt, site_idx) VALUES ".$vals ."ON DUPLICATE KEY UPDATE cnt = VALUES(cnt), pc_cnt = VALUES(pc_cnt), m_cnt = VALUES(m_cnt)";
    $strSQL = " INSERT INTO basic_refer_keyword_count (search_date, search_engine, cnt, pc_cnt, m_cnt, site_idx) VALUES ".$vals ."ON DUPLICATE KEY UPDATE cnt = VALUES(cnt), pc_cnt = VALUES(pc_cnt), m_cnt = VALUES(m_cnt)";
    $_LOGDB->query($strSQL);
}
