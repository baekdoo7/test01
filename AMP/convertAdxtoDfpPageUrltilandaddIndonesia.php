...
//구글 정보 뽑아오기
function getGoogleAd($str,$advIdx,$areaNm){
...
function makeTemplete($obj){
    ...
    $pageUrl  = $obj->pageUrl;
    $googleCd = '/'.$obj->fkey.'/'.$obj->pub.'/'.$obj->slot;
...
function runConvert(){
    ...
    $readDataQuery = "select a.adv_idx,a.html_code,b.area_nm from advertise_ad_change as a, th_convert_info as b where a.com_idx = 'a252ab7a-3306-4038-9475-3fb5001e4855' and a.network_adv_idx  = '41350b05-4415-44b2-8e17-b5fe52d1bd6e' and a.adv_idx = b.adv_idx; ";
    $resultSet     = $connGlobal->query($readDataQuery);
    ...
    while($row = $resultSet->fetch_assoc()){
       ...
       if(chkGoogleAd($row["html_code"])){
           $tmp = getGoogleAd($row["html_code"],$row["adv_idx"],$row["area_nm"]);
           if(!$tmp->error){
               $htmCode = makeTemplete($tmp);
               $updateQuery = "UPDATE advertise_ad_change SET html_code=\"".addslashes($htmCode)."\",network_adv_idx=\"9dbc26fa-bc8f-11e8-bbc3-02c31b446301\",net_adv_param01=\"".$tmp->fkey."\",net_adv_param02=\"".$tmp->pub."\",net_adv_param03=\"".$tmp->slot."\" WHERE adv_idx=\"".$row["adv_idx"]."\"";
               $connGlobal->query($updateQuery);

               $updateQuery2 = "UPDATE inventory_area_change ia2,(SELECT origin_areacd FROM inventory_area_ad iaa INNER JOIN inventory_area_change ia2 ON iaa.area_idx = ia2.area_idx WHERE iaa.adv_idx = '".$row["adv_idx"]."') tmp SET ia2.area_type = '9dbc26fa-bc8f-11e8-bbc3-02c31b446301' WHERE ia2.origin_areacd = tmp.origin_areacd  AND ia2.area_type = '41350b05-4415-44b2-8e17-b5fe52d1bd6e';";
