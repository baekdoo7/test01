 ...
 * AddItem
 * [π∞«∞µÓ∑œ¿ª ¡¶∞¯]
 * http://api.auction.co.kr/APIv1/Auctionservice.asmx?op=AddItem
 * 
...
class addItem
{
	...
	private $action = "http://www.auction.co.kr/APIv1/ShoppingService/AddItem";
	private $serverUrl = "https://api.auction.co.kr/APIv1/ShoppingService.asmx";	// Ω«º≠πˆ
	...
	private function processResponse($responseXml){
		if(stristr($responseXml, 'HTTP 404') || $responseXml == '') {
			...
			if (empty ($eleFaultcode)){
				...
				$eleCatalog = $responseDoc->getElementsByTagName('AddItemResult');
				
				for ($i = 0 ; $i < $eleCatalog->length ; $i++){
					echo "ItemID : ".$eleCatalog->item($i)->getAttribute("ItemID")."<BR>";
					echo "Fee : ".$this->getString($eleCatalog->item($i)->getAttribute("Fee"))." ";
					echo "<BR>";
				}
	...
	private function getString($value){
		return iconv("UTF-8", "EUC-KR", urldecode (htmlentities ($value, ENT_NOQUOTES, "UTF-8")));
	}
}	
?>